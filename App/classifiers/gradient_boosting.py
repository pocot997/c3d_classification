...
TRAIN_DIR = r"C:\Repos\Mgr\data_random_short\data_moments\complete"
TEST_DIR = r"C:\Repos\Mgr\data_random_short\data_moments\complete"
...
for attempt in attempts:
    ...
    if(counter == 0):
        with open(TRAIN_DIR + r"\gradient_boosting.txt", 'w') as file:
            file.write(content + '\n')
    else:
        with open(TRAIN_DIR + r"\gradient_boosting.txt", 'a') as file:
            file.write(content + '\n')
    ...
    attempt_end_time = dt.datetime.now()
    attempt_fit_time = attempt_end_time - attempt_start_time
    print(attempt_fit_time)
